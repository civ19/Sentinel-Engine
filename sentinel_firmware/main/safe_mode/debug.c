...
void sync_time(void) {
    // 1. Check if SNTP is already initialized/running
    if (esp_sntp_enabled()) {
        printf("SNTP already running, skipping configuration...\n");
    } else {
        // 2. Only configure if it's NOT running
        esp_sntp_setoperatingmode(SNTP_OPMODE_POLL);
        esp_sntp_setservername(0, "pool.ntp.org");
        esp_sntp_init();
    }
    
    // 3. Wait for sync (this part is safe to call regardless)
    int retry = 0;
    while (sntp_get_sync_status() == SNTP_SYNC_STATUS_RESET && ++retry < 10) {
        printf("Waiting for system time to sync... (%d/10)\n", retry);
